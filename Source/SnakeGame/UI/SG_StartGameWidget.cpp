...
void USG_StartGameWidget::NativeOnInitialized()
{
    ...
    InitComboBox(
        GridSizeComboBox,                                    //
        [&]() { return UserSettings->GridSizesOptions(); },  //
        [&]() { return UserSettings->CurrentGridSizeOption(); });
}

void USG_StartGameWidget::InitComboBox(const TObjectPtr<UComboBoxString>& ComboBox, TFunction<TArray<FString>()> OptionGetter, TFunction<FString()> CurrentOptionGetter)
{
    check(ComboBox);
    ComboBox->ClearOptions();
    for (const auto& Option : OptionGetter())
    {
